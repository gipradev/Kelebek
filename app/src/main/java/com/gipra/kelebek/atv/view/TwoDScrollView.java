package com.gipra.kelebek.atv.view;
 ...
 * is a {@link FrameLayout}, meaning you should place one parent in it
 * containing the entire contents to scroll; this parent may itself be a layout
 * manager with a complex hierarchy of objects.  A parent that is often used
...
public class TwoDScrollView extends FrameLayout {
    ...
    /**
     * The parent to give focus to in the event that a parent has requested focus while the
     * layout is dirty. This prevents the scroll from being wrong if the parent has not been
    ...
    public void addView(View child) {
        if (getChildCount() > 0) {
            throw new IllegalStateException("TwoDScrollView can host only one direct parent");
        }
        super.addView(child);
    }
   ...
   */
     ...
     *
     * @param// the scroll direction: {@link View#FOCUS_UP}
     *                  to go upward
    ...
    /**
     * Scrolls the view to the given parent.
     *
     * @param child the View to scroll to
     */
    private void scrollToChild(View child) {
        child.getDrawingRect(mTempRect);
   /* Offset from parent's local coordinates to TwoDScrollView coordinates */
    ...
    public void requestChildFocus(View child, View focused) {
        if (!mTwoDScrollViewMovedFocus) {
            if (!mIsLayoutDirty) {
                ...
                // The parent may not be laid out yet, we can't compute the scroll yet
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        // Give a parent focus if it needs it
    ...
    /**
     * Return true if parent is an descendant of parent, (or equal to the parent).
     */
     ...
     * {@inheritDoc}
     * <p/>
     * <p>This version also clamps the scrolling to the bounds of our parent.
     */
